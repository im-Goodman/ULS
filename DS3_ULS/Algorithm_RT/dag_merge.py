...
def _ranking_based_merge(*args, **kwargs):
    ...
    for sort_idx, dag_idx in enumerate(sorted_indices):
        ...
        terminal_node = \
            [node for node in args[dag_idx - 1].nodes() if not any(True for _ in args[dag_idx - 1].successors(node))][0]
...
def merge_2_dags(dag_1, dag_2):
    # 最大节点号
    max_node = max(dag_1)
    for node in list(dag_2.nodes()):
        flag_1 = False
        for mm in common.TaskQueues.outstanding.list:
            if mm.ID == node:
                flag_1 = True
                break
        for mm in common.TaskQueues.ready.list:
            if mm.ID == node:
                flag_1 = True
                break
        for mm in common.TaskQueues.executable.list:
            if mm.ID == node:
                flag_1 = True
                break
        if not flag_1:
            # print(node)
            # pass
            dag_2.remove_node(node)

    # print(dag_2.nodes)
    # 起始节点
    root_nodes = [node for node in dag_1.nodes() if not any(True for _ in dag_1.predecessors(node))] + [node for node in
                                                                                                        dag_2.nodes() if
                                                                                                        not any(
                                                                                                            True for _
                                                                                                            in
                                                                                                            dag_2.predecessors(
                                                                                                                node))]
    # 终止节点
    terminal_nodes = [node for node in dag_1.nodes() if not any(True for _ in dag_1.successors(node))] + [node for node
                                                                                                          in
                                                                                                          dag_2.nodes()
                                                                                                          if not any(
            True for _ in dag_2.successors(node))]

    dag = nx.compose(dag_1, dag_2)

    # print('1: ', dag_1.nodes)
    # print('2: ', dag_2.nodes)

    # 添加两个节点
    dag.add_node(max_node + 1)
    dag.add_node(max_node + 2)

    for node in root_nodes:
        dag.add_edge(max_node + 1, node)
        dag[max_node + 1][node]['weight'] = 0

    # 添加新终止节点
    for node in terminal_nodes:
        dag.add_edge(node, max_node + 2)
        dag[node][max_node + 2]['weight'] = 0

    # print(dag.nodes)
    return dag


def merge_2_dags_2(dag):
    root_node = dag.nodes[-2]
    terminal_node = dag.nodes[-1]
...
def merge_2_dags_1(dag):
    root_node = max(dag) - 1
    terminal_node = max(dag)
    for node in list(dag.nodes()):
        flag_1 = False
        for mm in common.TaskQueues.running.list:
            if mm.ID == node:
        ...
        if flag_1:
            ...
            for i in succ:
                dag.add_edge(root_node, i)
            dag.remove_node(node)
    for node in dag.successors(root_node):
        dag[root_node][node]['weight'] = 0

    # 添加新终止节点
    for node in dag.predecessors(terminal_node):
        dag[node][terminal_node]['weight'] = 0
    # print(dag.nodes)
    return dag
