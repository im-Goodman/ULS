...
class DTPMmodule:
    ...
    def evaluate_PE(self, resource, current_PE, timestamp):
        ...
        if self.timestamp_last_update_cluster[current_PE.cluster_ID] != timestamp and timestamp != 0:
            ...
            elif current_cluster.DVFS == 'imitation-learning' and self.timestamp_last_update_cluster.count(timestamp) == (len(self.timestamp_last_update_cluster) - 1):
                DTPM_policies.imitation_learning_policy(self.resource_matrix, self.PEs, self.env.now)
            elif current_cluster.DVFS == 'DyPO':
                DTPM_policies.DyPO_policy(self.resource_matrix, self.PEs, self.env.now)
            ...
            if timestamp % common.sampling_rate_temperature == 0 and self.timestamp_last_update_cluster.count(timestamp) == (len(self.timestamp_last_update_cluster) - 1):
                ...
                # print(common.current_temperature_vector)
                common.snippet_temp_list.append(max(common.current_temperature_vector))

                if common.enable_thermal_management and max(common.current_temperature_vector) >= common.DTPM_thermal_limit:
                    common.thermal_limit_violated = True
            ...
            if common.ClusterManager.cluster_list[current_PE.cluster_ID].type != 'MEM':
                if current_PE.process.count == 0:
                    ...
                    if DASH_Sim_utils.get_num_tasks_being_executed(common.ClusterManager.cluster_list[current_PE.cluster_ID], self.PEs) == 0:
                        if common.ClusterManager.cluster_list[current_PE.cluster_ID].type != 'ACC':
